    ...
    <div className="flex flex-col h-full w-full bg-slate-950 text-slate-200 overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
        ...
        <div className="md:hidden flex-1 bg-black flex flex-col items-center justify-center relative overflow-hidden min-h-[40vh]">
          {/* Mock Canvas Area */}
          <div 
            className="relative bg-slate-900 shadow-2xl overflow-hidden flex items-center justify-center border border-slate-800 h-full max-h-full"
          ...
          <div className="absolute bottom-4 inset-x-0 flex justify-center z-10 scale-90">
            <div className="bg-slate-900/80 backdrop-blur border border-slate-700 px-4 py-2 rounded-full flex items-center gap-4 shadow-xl">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => setCurrentTime(0)} className="text-slate-300 hover:text-white transition-colors p-1">
                  <SkipBack className="w-3.5 h-3.5" />
                </button>
                <button 
                  onClick={togglePlay}
                  className="w-8 h-8 bg-white text-slate-900 rounded-full flex items-center justify-center hover:scale-105 transition-transform"
                >
                  {isPlaying ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4 ml-0.5" />}
                </button>
                <button onClick={() => setCurrentTime(totalDuration)} className="text-slate-300 hover:text-white transition-colors p-1">
                ...
                <div className="grid grid-cols-2 md:grid-cols-1 gap-3">
                  {clips.map((clip, i) => (
                    <div key={clip.id} className="relative group rounded-lg overflow-hidden border border-slate-700 bg-slate-800">
                      <img src={clip.url} alt={clip.name} className="w-full h-16 md:h-20 object-cover opacity-60 group-hover:opacity-100 transition-opacity" />
                      <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/80 to-transparent p-2">
                        <p className="text-[10px] font-bold text-white leading-tight">{clip.name}</p>
              ...
              <div className="space-y-4">
                ...
                {selectedText && (
                  <div className="space-y-4 pt-4 border-t border-slate-800">
                    <div>
                      <label className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5 block">Edit Teks</label>
                      <textarea 
                    ...
                    <div>
                      <label className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5 block">Gaya TikTok</label>
                      <div className="grid grid-cols-2 gap-1.5 md:gap-2">
                        {(['tiktok-classic', 'tiktok-bordered', 'tiktok-neon', 'plain'] as const).map(style => (
                          <button
                            ...
                            className={`py-1.5 px-1 md:px-2 text-[9px] md:text-[10px] font-bold rounded border transition-colors truncate ${selectedText.style === style ? 'bg-cyan-900/40 border-cyan-500 text-cyan-400' : 'bg-slate-800 border-slate-700 text-slate-400 hover:bg-slate-700'}`}
                          >
                    ...
                    <div>
                      <label className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5 block">Warna Utama</label>
                      <div className="flex gap-2">
                        {['#ffffff', '#000000', '#ff0050', '#00f2fe', '#facc15'].map(color => (
                          <button
                            key={color}
                            onClick={() => setTexts(texts.map(t => t.id === selectedText.id ? {...t, color} : t))}
                            className={`w-6 h-6 rounded-full border-2 shadow-sm ${selectedText.color === color ? 'border-cyan-400' : 'border-slate-700/50'}`}
                    ...
                    <button 
                      ...
                      className="w-full py-2 bg-red-950/30 hover:bg-red-900/50 text-red-400 border border-red-900/50 text-xs font-bold rounded-lg flex items-center justify-center gap-2 transition-colors mt-2"
                    >
