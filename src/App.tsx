...
export default function App() {
  ...
  // Helper: Fetch or create invoice for current user
  const fetchOrCreateInvoice = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        if (data.invoice) {
          setUserInvoice(data.invoice);
        }
      }
    } catch (err) {
      console.warn('Error fetching or creating invoice:', err);
    } finally {
  ...
  const handleSendWhatsappConfirmation = () => {
    ...
    const invNum = userInvoice?.invoiceNumber || 'INV-NEURONA-PENDING';
    const name = userProfile?.namaLengkap || userInvoice?.userName || '';
    const email = userProfile?.email || userInvoice?.userEmail || '';
    const amountVal = userInvoice?.amount || priceSetting.promoPrice || 99000;
    const amountStr = `Rp ${amountVal.toLocaleString('id-ID')}`;

    const message = `Saya sudah melakukan pembayaran NEURONA.
...
Instruksi: Mohon lampirkan foto/screenshot bukti transfer Anda untuk proses aktivasi.`;
  ...
  // Trigger invoice fetch on currentUser / payment_instructions active tab
  useEffect(() => {
    if (currentUser && activeTab === 'payment_instructions') {
  ...
  const handleApproveUser = async (targetUid: string) => {
    try {
      ...
      // Update corresponding invoice document status to VERIFIED
      try {
        const invQuery = query(collection(db, 'invoices'));
        const invSnap = await getDocs(invQuery);
        invSnap.forEach(async (docSnap) => {
          const invData = docSnap.data();
          if (invData.userId === targetUid && invData.status === 'PENDING') {
        ...
        console.warn('Invoice update error on approve:', invErr);
      }
  ...
  const handleRejectUser = async (targetUid: string) => {
    try {
      ...
      // Update corresponding invoice document status to REJECTED
      try {
        const invQuery = query(collection(db, 'invoices'));
        const invSnap = await getDocs(invQuery);
        invSnap.forEach(async (docSnap) => {
        ...
        console.warn('Invoice update error on reject:', invErr);
      }
  ...
  // Admin Actions: Update Bank Info & Payment WhatsApp Settings
  const handleSaveBankSettings = async (e: React.FormEvent) => {
    ...
    if (!adminBankForm.bankName.trim() || !adminBankForm.accountNumber.trim() || !adminBankForm.accountHolder.trim()) {
      showToast('Harap lengkapi nama bank, nomor rekening, dan atas nama', 'error');
      return;
    }
        ...
        {isAdminPage ? (
                    ...
                    <form onSubmit={handleSaveBankSettings} className="space-y-6 max-w-xl">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <input 
                          ...
                          value={adminBankForm.whatsappNumber || ''}
                          onChange={(e) => setAdminBankForm({...adminBankForm, whatsappNumber: e.target.value})}
                          className="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/10 text-sm"
                        />
                        <p className="text-[11px] text-slate-500">Nomor WhatsApp ini menjadi tujuan tombol 'Kirim Bukti Pembayaran via WhatsApp' di faktur pendaftar.</p>
                      </div>
                      ...
                      >
                        Simpan Pengaturan Rekening & WhatsApp
                      </button>
                    </form>
                  </div>
                )}
          ...
          <div className="flex-1 flex flex-col">
            ...
            {activeTab === 'payment_instructions' && (
              <div className="flex-1 flex justify-center items-center px-4 py-12 bg-slate-50">
                <div className="w-full max-w-2xl bg-white p-6 md:p-8 rounded-2xl border border-slate-200/80 shadow-lg relative overflow-hidden">
                  
                  {/* Decorative Invoice Top Bar */}
                  <div className="absolute top-0 left-0 right-0 h-2 bg-gradient-to-r from-blue-600 via-cyan-500 to-teal-400"></div>

                  {/* Header & Status */}
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-100 mb-6">
                    <div>
                      <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-md bg-blue-50 text-blue-700 text-xs font-black tracking-wider uppercase mb-1">
                        Faktur Pembayaran Resmi
                      </div>
                      <h2 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2">
                        <span>INVOICE NEURONA</span>
                      </h2>
                      <div className="flex items-center gap-2 text-xs font-mono text-slate-500 mt-1">
                        <span>Nomor:</span>
                        <strong className="text-slate-800">{userInvoice?.invoiceNumber || 'INV-NEURONA-PENDING'}</strong>
                        <button
                          type="button"
                          onClick={() => {
                            if (userInvoice?.invoiceNumber) {
                              navigator.clipboard.writeText(userInvoice.invoiceNumber);
                              setCopiedInvoiceNum(true);
                              showToast('Nomor invoice disalin!', 'success');
                              setTimeout(() => setCopiedInvoiceNum(false), 2000);
                            }
                          }}
                          className="text-cyan-600 hover:text-cyan-700 p-1 rounded hover:bg-slate-100 transition-colors"
                          title="Salin Nomor Invoice"
                        >
                          {copiedInvoiceNum ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5" />}
                        </button>
                      </div>
                    </div>
                    ...
                    <div>
                      {userProfile?.status === 'REJECTED' ? (
                        <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-rose-50 border border-rose-200 text-rose-700 text-xs font-extrabold">
                          <XCircle className="w-4 h-4 text-rose-600" />
                          <span>DITOLAK</span>
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-amber-50 border border-amber-200 text-amber-800 text-xs font-extrabold animate-pulse">
                          <Clock className="w-4 h-4 text-amber-600" />
                  ...
                  <div className="bg-slate-50/80 p-4 rounded-xl border border-slate-100 mb-6 grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                    <div>
                      <span className="text-slate-400 font-bold uppercase tracking-wider block text-[10px]">Nama Pendaftar</span>
                      <span className="font-extrabold text-slate-800 text-sm">{userProfile?.namaLengkap || userInvoice?.userName || '-'}</span>
                    </div>
                    <div>
                      <span className="text-slate-400 font-bold uppercase tracking-wider block text-[10px]">Email</span>
                      <span className="font-bold text-slate-700 text-sm truncate block">{userProfile?.email || userInvoice?.userEmail || '-'}</span>
                    </div>
                    <div>
                      <span className="text-slate-400 font-bold uppercase tracking-wider block text-[10px]">WhatsApp</span>
                      <span className="font-bold text-slate-700 text-sm">{userProfile?.whatsapp || userInvoice?.userWhatsapp || '-'}</span>
                    </div>
                  </div>

                  {/* Itemized Price Summary */}
                  <div className="mb-6 bg-slate-900 text-white p-5 rounded-2xl shadow-sm">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pt-2 border-t border-slate-800">
                      <div>
                        <h4 className="font-extrabold text-base text-white">Akses Lifetime Storyboard AI (5 Studio)</h4>
                        <p className="text-xs text-slate-400 mt-0.5">Sekali Bayar Seumur Hidup • Bebas Biaya Bulanan/Tahunan</p>
                      </div>
                      <div className="text-right">
                        <p className="text-2xl font-black text-amber-400 tracking-tight">
                          Rp {(userInvoice?.amount || priceSetting.promoPrice || 99000).toLocaleString('id-ID')}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 mb-6 space-y-2.5 text-xs text-slate-600">
                    <div className="flex gap-2.5">
                      <span className="w-5 h-5 bg-blue-100 text-blue-700 font-extrabold rounded-full flex items-center justify-center shrink-0 text-[11px]">1</span>
                      <p className="leading-relaxed">
                        Lakukan transfer tepat sebesar <strong className="text-slate-800 font-black">Rp {(userInvoice?.amount || priceSetting.promoPrice || 99000).toLocaleString('id-ID')}</strong> ke rekening resmi di atas.
                      </p>
                    </div>
                    <div className="flex gap-2.5">
                      <span className="w-5 h-5 bg-blue-100 text-blue-700 font-extrabold rounded-full flex items-center justify-center shrink-0 text-[11px]">2</span>
                      <p className="leading-relaxed">
                        Setelah berhasil transfer, klik tombol <strong className="text-emerald-700 font-bold">"Kirim Bukti Pembayaran via WhatsApp"</strong> di bawah.
                      </p>
                    </div>
                    <div className="flex gap-2.5">
                      <span className="w-5 h-5 bg-blue-100 text-blue-700 font-extrabold rounded-full flex items-center justify-center shrink-0 text-[11px]">3</span>
                      <p className="leading-relaxed">
                        Lampirkan foto/screenshot bukti transfer di percakapan WhatsApp. Admin akan memverifikasi & mengaktifkan akun Anda dalam 5-15 menit.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <button
                      ...
                      className="w-full py-4 px-6 bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold text-sm sm:text-base rounded-2xl shadow-lg shadow-emerald-600/25 transition-all hover:scale-[1.01] flex items-center justify-center gap-2"
                    >
                    ...
                    <div className="flex flex-col sm:flex-row gap-2.5 pt-1">
                      <button
                        type="button"
                        onClick={async () => {
                          showToast('Memeriksa status aktivasi...', 'info');
                          if (currentUser) {
                            try {
                              ...
                              if (userSnap.exists()) {
                                ...
                                setUserProfile(profile);
                                if (profile.status === 'ACTIVE') {
                                  showToast('Selamat! Akun Anda telah diaktifkan oleh admin.', 'success');
                                  setActiveTab('user_dashboard');
                                  return;
                                }
                              }
                      ...
                      >
                        <Loader2 className={`w-3.5 h-3.5 ${invoiceLoading ? 'animate-spin' : ''}`} />
                      ...
                      <button
                        type="button"
                        onClick={handleLogout}
                        className="py-2.5 px-5 border border-slate-200 hover:bg-slate-50 text-slate-600 font-bold text-xs rounded-xl transition-colors"
                      >
                        Keluar
                      </button>
                    </div>
